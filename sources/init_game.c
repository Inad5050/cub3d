...
/*   Updated: 2024/12/30 21:01:26 by dangonz3         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_game(t_cub *c)
{
	...
	c->win = mlx_new_window(c->mlx, WIN_WIDHT, WIN_HEIGHT, "Cube3d"); //crea la ventana
	if (!c->win)
		return (c_error("Couldn't initiate c->win", c), EXIT_FAILURE);
	c->p = ft_calloc(1, sizeof(t_plyr)); //inicializa la estructura t_player
	if (!c->p)
